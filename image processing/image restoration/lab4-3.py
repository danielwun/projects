...
def make_gfilter( sigma, n ):
    ...
    for x in X :
        for y in X :
            gfilter[ int(y+n/2), int(x+n/2) ] = gaussian( sigma, x, y)
    return gfilter
...
def PSNR( image, target ):
    ...
    mse = np.array(mse)/(length*width)
    mse = np.sum( 10*np.log(1/mse) )
    return mse
...
def main():
    target = np.array( io.imread("input3_ori.bmp"), dtype = float)/255
    image = np.array( io.imread("input3.bmp"), dtype=float)      # RGB
    gaussian = make_gfilter( 4, 31 )
    ...
    a = 20*np.log(np.abs(np.fft.fftshift( imageg )))
    #showgraph(a)
    ...
    b = np.fft.fftshift( egaufft+1 )  #put frequency (0. , 0.) to middle
    ...
    imager = np.divide( imager, egaufft )
    imageg = np.divide( imageg, egaufft )
    imageb = np.divide( imageb, egaufft )
    '''
    imager = wiener( egaufft, imager, 0.05 )
    imageg = wiener( egaufft, imageg, 0.05 )
    imageb = wiener( egaufft, imageb, 0.05 )

    a = 20*np.log(np.abs(np.fft.fftshift( imageg ))+1)
    io.imshow(a,cmap='gray')
    plt.show()
