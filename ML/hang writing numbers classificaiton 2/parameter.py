...
def showGraph( decisiontree, num, time ) :
    ...
    graph.write_png("test"+str(time)+'-'+str(num)+".png")
...
def decisionTree(fraction, nd, t_learn, nd_test, time ):        
    forest = []
    for i in range(100):
        ...
        #if i%49==0 :
         #   showGraph( dt.fit( x, y), i/49, time )
    result = np.zeros(1) ;
    for t in forest :
...
def main():
    ...
    while a==1 :
        z = np.ones(hlayer)
        y = np.zeros(5)
        wzx = np.random.rand(hlayer-1,785)*0.001 #weight between z and x
        wyz = np.random.rand(5,hlayer)*0.001   #Weight between y and z 
        for i in range(250):
            wzx,wyz = neural_Learning( x, wzx, wyz, z, y, t )
        outputFile( wzx , time)
        #wzx = readParameter('parameter1.csv')
        ...
        result = decisionTree( fraction, nd, t_learn, nd_test, time)
        print('time = ',time,' error rate= ',accuracy(result, t_test ))
        time = time+1
